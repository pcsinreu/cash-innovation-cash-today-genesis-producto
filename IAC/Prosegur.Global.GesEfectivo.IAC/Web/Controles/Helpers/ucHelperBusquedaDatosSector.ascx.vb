...
Public Class ucHelperBusquedaDatosSector
...
#Region "[PROPRIEDADES]"
    ...
    Public Property PopUpAbierto As Boolean
        Get
            Return ViewState(Me.ID + "PopUpAbiertoSector")
        End Get
        Set(value As Boolean)
            ViewState(Me.ID + "PopUpAbiertoSector") = value
        End Set
    End Property
    ...
    Public Property ucPopup() As Popup2
        Get
            If _ucPopUp Is Nothing Then
                ...
                _ucPopUp.ID = Me.ID & "_ucPopupSector"
    ...
    Private WithEvents _ucBusquedaAvanzada As ucHelperBusquedaAvanzadaSector
    Public Property ucBusquedaAvanzada() As ucHelperBusquedaAvanzadaSector
        Get
            If _ucBusquedaAvanzada Is Nothing Then
                _ucBusquedaAvanzada = LoadControl("~\Controles\Helpers\ucHelperBusquedaAvanzadaSector.ascx")
                _ucBusquedaAvanzada.ID = Me.ID & "_ucHelperBusquedaAvanzadaSector"
        ...
        End Get
        Set(value As ucHelperBusquedaAvanzadaSector)
...
#Region "[OVERRIDES]"
    ...
    Protected Sub ConfigurarControle_BusquedaAvanzada(Optional EnviarRegistrosSelecionados As Boolean = True)
        ...
        ucPopup.AbrirPopup()
        If Not String.IsNullOrEmpty(txtCodigo.Text) OrElse Not String.IsNullOrEmpty(txtDescripcion.Text) Then
            ucBusquedaAvanzada.Codigo = txtCodigo.Text
            ucBusquedaAvanzada.Descricao = txtDescripcion.Text
            ucBusquedaAvanzada.ExisteListado = False
        ElseIf Me.RegistrosSelecionados IsNot Nothing AndAlso Me.RegistrosSelecionados.DatosRespuesta IsNot Nothing Then
            ...
            ucBusquedaAvanzada.ExisteListado = True
        Else
            ucBusquedaAvanzada.ExisteListado = False
        End If
...
#Region "[EVENTOS]"
    ...
    Protected Sub ucBusquedaAvanzada_Fechado(sender As Object, e As PopupEventArgs) Handles _ucBusquedaAvanzada.Fechado
        Try
            ...
            If e.Resultado IsNot Nothing Then
                ...
                ElseIf Me.ucBusquedaAvanzada.eventoExecutado = EventoPopupSector.FecharClick Then
    ...
    ''' <summary>
    ''' Método referente ao evento de Exclusão dos itens selecionados no ListBox.
    ''' </summary>    
    Protected Sub imgButtonRemove_Click(sender As Object, e As ImageClickEventArgs) Handles imgButtonRemove.Click
        Try
            Dim dadosRemover() As Integer = lstBoxBusqueda.GetSelectedIndices
            If (dadosRemover.Count > 0) Then
                While dadosRemover.Count > 0
                    Dim item = lstBoxBusqueda.Items(dadosRemover(0))
                    RegistrosSelecionados.DatosRespuesta.RemoveAll(Function(d) d.Identificador = item.Value)
                    lstBoxBusqueda.Items.Remove(item)
                    dadosRemover = lstBoxBusqueda.GetSelectedIndices
                End While

                If (lstBoxBusqueda.Items.Count < 1) Then Me.ExibirMultDados(False)
            Else
                ScriptManager.RegisterClientScriptBlock(Me.Page, Me.GetType, "erro", _
                                              Base.CriarChamadaMensagemErro(Traduzir("msg_remover_nao_selecionado"), Nothing), True)
...
#Region "[METODOS]"
    ...
    Private Sub ConfigurarCampos()
        ...
        If Not String.IsNullOrEmpty(txtCodigo.Text) AndAlso Not String.IsNullOrEmpty(txtDescripcion.Text) AndAlso ControleHabilitado Then
            ...
            dvButtonLimpaCampo.Style.Item("margin") = "0px 0px 0px 0px"
        Else
            dvButtonLimpaCampo.Style.Item("display") = "none"
        End If
        dvDescripcion.Style.Item("display") = "none"
        dvBoxBusqueda.Style.Item("display") = "none"
        dvButtonRemove.Style.Item("display") = "none"
        ...
        If ControleHabilitado Then
            ...
            dvButtonBusqueda.Style.Item("margin") = "0px 0px 0px 0px"
            ...
            imgButtonRemove.Enabled = ControleHabilitado
            lstBoxBusqueda.Enabled = ControleHabilitado

            dvDescripcion.Style.Item("display") = "block"
            Me.imgButtonRemove.Style.Add("cursor", "pointer")


            ucHelper.Style.Item("width") = "480px !important"
        ...
        Try
            ...
            If RegistrosSelecionados IsNot Nothing AndAlso RegistrosSelecionados.DatosRespuesta IsNot Nothing Then
                ...
                If MultiSelecao Then
                    Me.ExibirMultDados(RegistrosSelecionados.DatosRespuesta.Count >= 1)
                    lstBoxBusqueda.Items.Clear()
                    ' Preenche Listbox
                    For index = RegistrosSelecionados.DatosRespuesta.Count - 1 To 0 Step -1
                        ...
                        lstBoxBusqueda.Items.Add(New ListItem() With {.Value = item.Identificador, .Text = item.Codigo + " - " + item.Descricao})
                    Next
                Else
                    Me.PreencherCodigoDescricao()
                End If
    ...
    Protected Sub ExibirMultDados(visivel As Boolean)
        'lstBoxBusqueda.Items.Clear()
        dvBoxBusqueda.Style.Item("display") = If(visivel, "block", "none")
        dvButtonRemove.Style.Item("display") = If(visivel, "block", "none")
    End Sub

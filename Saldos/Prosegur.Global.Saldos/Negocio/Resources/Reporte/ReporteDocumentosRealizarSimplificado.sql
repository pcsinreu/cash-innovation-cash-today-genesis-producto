SELECT distinct total.iddocumento
  ...
  FROM (SELECT DISTINCT dc.iddocumento
         ...
         LEFT JOIN pd_bulto bu
            ON dc.iddocumento = bu.iddocumento
          LEFT JOIN pd_banco bd
            ON dc.idbancodeposito = bd.idbanco
          LEFT JOIN pd_cliente bdc
            ON bd.idbanco = bdc.idcliente
          LEFT JOIN pd_banco b
            ON dc.idbanco = b.idbanco
          LEFT JOIN pd_cliente bc
            ON b.idbanco = bc.idcliente
          LEFT JOIN pd_cliente cd
            ON dc.idclientedestino = cd.idcliente
          LEFT JOIN pd_cliente co
            ON dc.idclienteorigen = co.idcliente
          LEFT JOIN pd_centroproceso cpd
            ON dc.idcentroprocesodestino = cpd.idcentroproceso
          LEFT JOIN pd_centroproceso cpo
            ON dc.idcentroprocesoorigen = cpo.idcentroproceso
          INNER JOIN pd_formulario f
            ON dc.idformulario = f.idformulario
          LEFT JOIN pd_formulariousuario fu
            ON f.idformulario = fu.idformulario
           AND (fu.idusuario IS NULL OR fu.idusuario = :idusuario OR :idusuario = 0)
         WHERE 0 = 0 
           {0}
           {1}
       ) total
 WHERE (total.numcomprobante >= :numcomprobantedesde OR LENGTH(:numcomprobantedesde) IS NULL)
   AND (total.numcomprobante <= :numcomprobantehasta OR LENGTH(:numcomprobantehasta) IS NULL)  
   AND ((:distinguirporvistadestinatario = 0 AND
       ((total.idcentroprocesoorigen = :idcentroproceso AND
       INSTR(:idsestadoscomprobanteemitido, '|' || total.idestadocomprobante || '|') > 0) OR
       (NVL(total.idcentroprocesodestino, total.idcentroprocesoorigen) = :idcentroproceso AND
       INSTR(:idsestadoscomprobanterecibido, '|' || total.idestadocomprobante || '|') > 0))) OR
       (:distinguirporvistadestinatario = 1 AND
       ((:vistadestinatario = 0 AND total.idcentroprocesoorigen = :idcentroproceso AND
       INSTR(:idsestadoscomprobanteemitido, '|' || total.idestadocomprobante || '|') > 0) OR
       (:vistadestinatario = 1 AND NVL(total.idcentroprocesodestino, total.idcentroprocesoorigen) = :idcentroproceso AND
       INSTR(:idsestadoscomprobanterecibido, '|' || total.idestadocomprobante || '|') > 0))))
   AND esgrupo = 0
   AND (INSTR(:idsformulariosrestriccion, '|' || total.idformulario || '|') > 0 OR
       LENGTH(:idsformulariosrestriccion) IS NULL)
   AND (:distinguirporreenvio = 0 OR (:distinguirporreenvio = 1 AND total.reenviado = :reenviado))
   AND (:distinguirpordisponibilidad = 0 OR (:distinguirpordisponibilidad = 1 AND total.disponible = :disponible))
   AND (:distinguirporbultos = 0 OR (:distinguirporbultos = 1 AND total.conbultos = :conbultos))
   AND (:distinguirporvalores = 0 OR (:distinguirporvalores = 1 AND total.convalores = :convalores))
   AND ((:contomados = 0 AND (NOT (SELECT COUNT(1)
                                     FROM pd_documentocabecera idc
                                    WHERE idc.idorigen = total.iddocumento) > 0)) OR (:contomados = 1))
   AND (:distinguirporsustitucion = 0 OR (:distinguirporsustitucion = 1 AND total.sustituido = :sustituido))
   {2}
 ORDER BY total.numexterno
         ,total.numcomprobante

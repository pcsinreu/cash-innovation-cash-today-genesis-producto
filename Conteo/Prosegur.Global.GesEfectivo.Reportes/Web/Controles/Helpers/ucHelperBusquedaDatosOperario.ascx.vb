...
Public Class ucHelperBusquedaDatosOperario
...
#Region "[PROPRIEDADES]"
    ...
    Private WithEvents _ucBusquedaAvanzada As ucHelperBusquedaAvanzadaOperario
    Public Property ucBusquedaAvanzada() As ucHelperBusquedaAvanzadaOperario
        Get
            If _ucBusquedaAvanzada Is Nothing Then
                _ucBusquedaAvanzada = LoadControl("~\Controles\Helpers\ucHelperBusquedaAvanzadaOperario.ascx")
                _ucBusquedaAvanzada.ID = Me.ID & "_ucHelperBusquedaAvanzadaOperario"
        ...
        End Get
        Set(value As ucHelperBusquedaAvanzadaOperario)
...
#Region "[OVERRIDES]"
    ...
    Protected Sub ConfigurarControle_BusquedaAvanzada(Optional EnviarRegistrosSelecionados As Boolean = True)
        ...
        If EnviarRegistrosSelecionados Then
            ...
            If Me.RegistrosSelecionados IsNot Nothing AndAlso Me.RegistrosSelecionados IsNot Nothing Then
                ucBusquedaAvanzada.RegistrosTemporarios = Me.RegistrosSelecionados
            End If
    ...
    Public Overrides Sub LimparCampos()
        ...
        If (MultiSelecao AndAlso (Me.RegistrosSelecionados IsNot Nothing AndAlso Me.RegistrosSelecionados IsNot Nothing AndAlso Me.RegistrosSelecionados.Count > 1)) Then
...
#Region "[EVENTOS]"
    ...
    Protected Sub imgButtonBusqueda_Click(sender As Object, e As ImageClickEventArgs) Handles imgButtonBusqueda.Click
        Try
            If Session("DelegacaoEcolhida") Is Nothing Then
                NotificarErro(New Exception("Favor informar o campo Delegação"))
                Exit Sub
            End If
    ...
    Protected Sub ucBusquedaAvanzada_Fechado(sender As Object, e As PopupEventArgs) Handles _ucBusquedaAvanzada.Fechado
        Try
            ...
            If e.Resultado IsNot Nothing Then
                Dim RespuestaPopUp As ContractoServ.GetUsuariosDetail.UsuarioColeccion = DirectCast(e.Resultado, ContractoServ.GetUsuariosDetail.UsuarioColeccion)
                If (Me.ucBusquedaAvanzada.eventoExecutado = EventoPopup.CodigoTextChanged OrElse Me.ucBusquedaAvanzada.eventoExecutado = EventoPopup.DescripcionTextChanged) Then
                    'se é multselecção
                    ...
                    If (Me.RegistrosSelecionados IsNot Nothing AndAlso Me.RegistrosSelecionados IsNot Nothing AndAlso Me.RegistrosSelecionados.Count > 0) AndAlso _
                        (RespuestaPopUp IsNot Nothing AndAlso RespuestaPopUp IsNot Nothing AndAlso RespuestaPopUp.Count > 0) Then

                        For Each dados In RespuestaPopUp
                            If Not Me.RegistrosSelecionados.Exists(Function(d) d.Login = dados.Login) Then
                                'Insere na primeira posição
                                Me.RegistrosSelecionados.Insert(0, dados)
                            End If
                        Next
                    Else
                ...
                ElseIf Me.ucBusquedaAvanzada.eventoExecutado = EventoPopup.FecharClick Then
                    ...
                    If executarScript Then
                        ...
                        If (Me.RegistrosSelecionados IsNot Nothing AndAlso Me.RegistrosSelecionados IsNot Nothing AndAlso Me.RegistrosSelecionados.Count = 1) Then
                            'Verifica se o textbox codigo ou descrição está preenchido, se sim, então deve limpar os campos...
                            If Not String.IsNullOrEmpty(Me.txtCodigo.Text) AndAlso String.IsNullOrEmpty(Me.txtDescripcion.Text) Then
                                strCodigo = Me.RegistrosSelecionados(0).Nombre
                                strDescripcion = Me.RegistrosSelecionados(0).Apellido1
                            ElseIf String.IsNullOrEmpty(Me.txtCodigo.Text) AndAlso Not String.IsNullOrEmpty(Me.txtDescripcion.Text) Then
                                strCodigo = Me.RegistrosSelecionados(0).Nombre
                                strDescripcion = Me.RegistrosSelecionados(0).Apellido1
                            End If
...
#Region "[METODOS]"
    ...
    Public Sub ExibirDados(Optional excutaEventoAtualizar As Boolean = True)
        Try
            ...
            If RegistrosSelecionados IsNot Nothing AndAlso RegistrosSelecionados.Count > 0 Then

                If MultiSelecao AndAlso (RegistrosSelecionados.Count > 1) Then
                    Me.ExibirMultDados(True)

                    ' Preenche Listbox
                    For index = RegistrosSelecionados.Count - 1 To 0 Step -1
                        Dim item As ContractoServ.GetUsuariosDetail.Usuario = RegistrosSelecionados(index)
                    Next
                Else
                    Me.PreencherCodigoDescricao()
                End If
            ...
            If (excutaEventoAtualizar) Then
                RaiseEvent UpdatedControl()
            Else
                Me.LimparCampos()
            End If
    ...
    Protected Sub PreencherCodigoDescricao()
        ...
        If RegistrosSelecionados IsNot Nothing AndAlso RegistrosSelecionados IsNot Nothing _
            AndAlso RegistrosSelecionados.Count = 1 Then

            RemoveHandler txtCodigo.TextChanged, AddressOf txtCodigo_TextChanged
            RemoveHandler txtDescripcion.TextChanged, AddressOf txtDescripcion_TextChanged

            txtCodigo.Text = RegistrosSelecionados(0).Login
            txtDescripcion.Text = RegistrosSelecionados(0).Nombre

            If Not String.IsNullOrEmpty(txtCodigo.Text) AndAlso ControleHabilitado Then
                dvButtonLimpaCampo.Style.Item("display") = "block"
            End If

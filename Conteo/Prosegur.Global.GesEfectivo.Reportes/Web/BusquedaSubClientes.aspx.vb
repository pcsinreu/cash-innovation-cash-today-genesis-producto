        ...
        Try
            ...
            If Not Page.IsPostBack Then
                ...
                If Not String.IsNullOrEmpty(Request.QueryString("codSubCliente")) Then
                    ...
                    txtCodigo.Text = Request.QueryString("codSubCliente")
...
#Region "[PROPRIEDADES]"
    ...
    ''' <summary>
    ''' Armazena os subclientes encontrados na busca.
    ''' </summary>
    ''' <value></value>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Property SubClientes() As IAC.ContractoServicio.Utilidad.GetComboSubclientesByCliente.SubClienteColeccion
        Get
            If ViewState("VSSubClientes") Is Nothing Then
                ViewState("VSSubClientes") = New IAC.ContractoServicio.Utilidad.GetComboClientes.ClienteColeccion
            End If
    ...
    ''' <summary>
    ''' Armazena o subcliente selecionado no grid.
    ''' </summary>
    ''' <value></value>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Property SubClienteSelecionado() As IAC.ContractoServicio.Utilidad.GetComboSubclientesByCliente.SubCliente
        Get
            Return Session("SubClienteSelecionado")
        End Get
        Set(value As IAC.ContractoServicio.Utilidad.GetComboSubclientesByCliente.SubCliente)
            Session("SubClienteSelecionado") = value
        End Set
    End Property
...
#Region "[EVENTOS GRIDVIEW]"
    ...
    Private Sub ProsegurGridView1_RowCreated(sender As Object, e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles ProsegurGridView.RowCreated
        ...
        Try
            ...
            If e.Row.RowType = DataControlRowType.Header Then
                CType(e.Row.Cells(0).Controls(0).Controls(0).Controls(0).Controls(0), Label).Text = Traduzir("009_lbl_gdr_codigo")
                ...
                CType(e.Row.Cells(1).Controls(0).Controls(0).Controls(0).Controls(0), Label).Text = Traduzir("009_lbl_gdr_descripcion")
    ...
    Protected Sub ProsegurGridView1_EPreencheDados(sender As Object, e As EventArgs) Handles ProsegurGridView.EPreencheDados
        ...
        Try
            ...
            Dim objRespuesta As IAC.ContractoServicio.Utilidad.GetComboSubclientesByCliente.Respuesta

            ' obter valores posibles
            objRespuesta = GetComboSubClientes()
            ...
            If objRespuesta.Clientes IsNot Nothing AndAlso _
               ...
               objRespuesta.Clientes(0).SubClientes.Count > 0 Then
                ...
                objDT = ProsegurGridView.ConvertListToDataTable(objRespuesta.Clientes(0).SubClientes)
    ...
    Private Sub btnAceptar_Click(sender As Object, e As System.EventArgs) Handles btnAceptar.Click
        ...
        Try
            ...
            If ProsegurGridView.getValorLinhaSelecionada = String.Empty AndAlso CampoObrigatorio = True Then
                strErro.Append(Traduzir("009_seleccione_un_subcliente") & Constantes.LineBreak)
            End If
            ...
            SalvarSubClienteSelecionado(ProsegurGridView.getValorLinhaSelecionada)
...
#Region "[MÉTODOS]"
    ...
    Private Function GetComboSubClientes() As IAC.ContractoServicio.Utilidad.GetComboSubclientesByCliente.Respuesta
        ...
        If Not String.IsNullOrEmpty(Request.QueryString("codCliente")) Then
            ...
            objPeticion.CodigosClientes.Add(Request.QueryString("codCliente").ToString())
        End If


        If Not String.IsNullOrEmpty(txtCodigo.Text.Trim) Then
            objPeticion.CodigoSubcliente = txtCodigo.Text.Trim.ToUpper
        End If

        If Not String.IsNullOrEmpty(txtDescricao.Text.Trim) Then
            objPeticion.DescripcionSubcliente = txtDescricao.Text.Trim.ToUpper
        End If
    ...
    Private Sub PreencherGridSubClientes()
        ...
        If objRespuesta.Clientes IsNot Nothing AndAlso _
           ...
           objRespuesta.Clientes(0).SubClientes.Count > 0 Then
            ...
            SubClientes = objRespuesta.Clientes(0).SubClientes

            ' Se existe apena um cliente
            If (SubClientes.Count = 1) Then
                ' Salva os dados do subcliente
                SalvarSubClienteSelecionado(SubClientes.First.Codigo)
            Else
                ' converter objeto para datatable
                Dim objDt As DataTable = ProsegurGridView.ConvertListToDataTable(objRespuesta.Clientes(0).SubClientes)
    ...
    ''' por outras telas que chamam a tela de busca de subclientes.
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub SalvarSubClienteSelecionado(codigoSubCliente As String)

        Dim objSubCliente As New IAC.ContractoServicio.Utilidad.GetComboSubclientesByCliente.SubCliente

        Dim pesquisa = From c In SubClientes _
                       Where c.Codigo = codigoSubCliente _
        ...
        SubClienteSelecionado = objSubCliente

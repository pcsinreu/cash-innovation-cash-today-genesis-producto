...
Namespace Integracion
    Public Class AccionAltaMovimientosMoveIn
        Public Shared Function Ejecutar(peticion As AltaMovimientosMoveIn.Peticion) As AltaMovimientosMoveIn.Respuesta
            ...
            End If
            Logeo.Log.Movimiento.Logger.GenerarIdentificador(codigoPais, "AltaMovimientosMoveIn", identificadorLlamada)
            If Not String.IsNullOrWhiteSpace(identificadorLlamada) Then

                Logeo.Log.Movimiento.Logger.IniciaLlamada(identificadorLlamada, "AltaMovimientosMoveIn", Comon.Util.VersionCompleta, AccesoDatos.Util.ToXML(peticion), codigoPais, AccesoDatos.Util.ToXML(peticion).GetHashCode)
            End If

            ' Validar campos obligatorios
            If ValidarPeticion(peticion, respuesta) Then
                Try
                    If String.IsNullOrEmpty(peticion.Configuracion.Usuario) Then peticion.Configuracion.Usuario = "SERVICIO_ALTA_MOVIMIENTOS_MOVEIN"

                    TiempoParcial = Now

                    AccesoDatos.GenesisSaldos.Movimientos.MoveIn.AltaMovimientosMoveIn(identificadorLlamada, peticion, respuesta, log)
        ...
        Private Shared Sub TratarRespuesta(peticion As AltaMovimientosMoveIn.Peticion, respuesta As AltaMovimientosMoveIn.Respuesta)
        ...
        Private Shared Function ValidarPeticion(ByRef peticion As AltaMovimientosMoveIn.Peticion, ByRef respuesta As AltaMovimientosMoveIn.Respuesta) As Boolean
            ...
            Return resp
        End Function
        Public Shared Function ValidarToken(ByVal peticion As AltaMovimientosMoveIn.Peticion,
                                    ByRef respuesta As AltaMovimientosMoveIn.Respuesta) As Boolean

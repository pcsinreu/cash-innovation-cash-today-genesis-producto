...
Imports Prosegur.Genesis.ContractoServicio.Contractos.Integracion.AltaMovimientosBalance
Imports System.Data
...
Namespace Integracion
    Public Class AccionAltaMovimientosBalance
        Public Shared Function Ejecutar(peticion As AltaMovimientosBalance.Peticion) As AltaMovimientosBalance.Respuesta
            ...
            End If
            Logeo.Log.Movimiento.Logger.GenerarIdentificador(codigoPais, "AltaMovimientosBalance", identificadorLlamada)
            If Not String.IsNullOrWhiteSpace(identificadorLlamada) Then
                Dim movimientoConActualId = peticion.Movimientos.Where(Function(x) Not String.IsNullOrEmpty(x.ActualId)).First()


                Logeo.Log.Movimiento.Logger.IniciaLlamada(identificadorLlamada, "AltaMovimientosBalance", Comon.Util.VersionCompleta, AccesoDatos.Util.ToXML(peticion), codigoPais, AccesoDatos.Util.ToXML(peticion).GetHashCode)
            End If

            ' Validar campos obligatorios
            If ValidarPeticion(peticion, respuesta) Then
                Try
                    If String.IsNullOrEmpty(peticion.Configuracion.Usuario) Then peticion.Configuracion.Usuario = "SERVICIO_ALTA_MOVIMIENTOS_BALANCE"

                    TiempoParcial = Now

                    AccesoDatos.GenesisSaldos.Movimientos.Balance.AltaMovimientosBalance(identificadorLlamada, peticion, respuesta, log)
                ...
                Catch ex As Excepcion.NegocioExcepcion
                    ...
                    respuesta.Resultado.Detalles.Add(New Salida.Detalle With {.Codigo = ex.Codigo, .Descripcion = ex.Descricao})
                Catch ex As Exception
        ...
        Private Shared Sub TratarRespuesta(peticion As AltaMovimientosBalance.Peticion, respuesta As AltaMovimientosBalance.Respuesta)
        ...
        Private Shared Function ValidarPeticion(ByRef peticion As AltaMovimientosBalance.Peticion, ByRef respuesta As AltaMovimientosBalance.Respuesta) As Boolean
            ...
            Return resp
        End Function
        Public Shared Function ValidarToken(ByVal peticion As AltaMovimientosBalance.Peticion,
                                    ByRef respuesta As AltaMovimientosBalance.Respuesta) As Boolean

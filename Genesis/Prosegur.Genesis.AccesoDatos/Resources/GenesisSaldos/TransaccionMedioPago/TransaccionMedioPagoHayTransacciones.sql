...
WHERE D.FYH_PLAN_CERTIFICACION <= []FYH_PLAN_CERTIFICACION AND TE.FYH_PLAN_CERTIFICACION <= []FYH_PLAN_CERTIFICACION AND D.BOL_CERTIFICADO = []BOL_CERTIFICADO AND CL.COD_CLIENTE = []COD_CLIENTE
AND TP.BOL_CERTIFICACION = 1 AND NOT EXISTS (SELECT CER.OID_CERTIFICADO FROM SAPR_TCERTIFICADO CER WHERE CER.OID_CERTIFICADO = TE.OID_CERTIFICADO AND CER.COD_ESTADO = 'DE')
{0}

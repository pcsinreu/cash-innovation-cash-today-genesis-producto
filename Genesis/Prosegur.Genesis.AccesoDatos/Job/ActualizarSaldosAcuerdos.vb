...
Namespace Genesis.Job
    Public Class ActualizarSaldosAcuerdos

        Public Shared Sub Ejecutar(identificadorLlamada As String, peticion As ContractoServicio.Contractos.Job.ActualizarSaldosAcuerdos.Peticion,
                          ByRef respuesta As ContractoServicio.Contractos.Job.ActualizarSaldosAcuerdos.Respuesta,
        ...
        Private Shared Sub PoblarRespuesta(ds As DataSet, respuesta As ContractoServicio.Contractos.Job.ActualizarSaldosAcuerdos.Respuesta)
            If ds IsNot Nothing AndAlso ds.Tables.Count > 0 Then

                ' Validaciones
                If ds.Tables.Contains("validaciones") AndAlso ds.Tables("validaciones").Rows.Count > 0 Then

                    If respuesta.Detalles Is Nothing Then respuesta.Detalles = New List(Of ContractoServicio.Contractos.Job.ActualizarSaldosAcuerdos.Salida.Detalle)
                    For Each row As DataRow In ds.Tables("validaciones").Rows
                        respuesta.Detalles.Add(New ContractoServicio.Contractos.Job.ActualizarSaldosAcuerdos.Salida.Detalle With {.Codigo = Util.AtribuirValorObj(row(0), GetType(String)), .Descripcion = Util.AtribuirValorObj(row(1), GetType(String))})
                    Next
                End If
        ...
        Private Shared Function ColectarPeticion(peticion As ContractoServicio.Contractos.Job.ActualizarSaldosAcuerdos.Peticion, identificadorLlamada As String) As SPWrapper
            Dim SP As String = String.Format("SAPR_PJOB_{0}.sactualizar_saldo_acuerdo", Prosegur.Genesis.Comon.Util.Version)
            ...
            spw.AgregarParam("par$bol_forzar_calculo", ProsegurDbType.Logico, peticion.ForzarCalculoDiaActual)

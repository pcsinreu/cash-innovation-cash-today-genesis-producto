...
Imports Prosegur.Genesis.ContractoServicio.Contractos.Integracion.AltaMovimientosCashOut
...
Namespace GenesisSaldos.Movimientos
    Public Class CashOut
        Public Shared Sub AltaMovimientosCashOut(peticion As Peticion, respuesta As Respuesta, log As StringBuilder, identificadorLlamada As String)
            ...
            Try
                ...
                If peticion.Movimientos IsNot Nothing AndAlso peticion.Movimientos.Count > 0 Then
                    TiempoParcial = Now
                    Dim movimientosIndex As New Dictionary(Of Integer, Entrada.MovimientoCashOut)
                    spw = sconfigurarMovimientosCashOut(peticion, movimientosIndex, identificadorLlamada)
                    ...
                    TiempoParcial = Now
                    PoblarMovimientosCashOut(ds, respuesta, movimientosIndex)
        ...
        Private Shared Sub PoblarMovimientosCashOut(ds As DataSet, respuesta As Respuesta, movimientosIndex As Dictionary(Of Integer, Entrada.MovimientoCashOut))
            If respuesta.Movimientos Is Nothing Then respuesta.Movimientos = New List(Of Salida.MovimientoCashOut)
                    ...
                    For Each row As DataRow In ds.Tables("validaciones").DefaultView.ToTable().Rows
                        ...
                        If Not (rowIndex = 2040010027 Or rowIndex = 2040010026) Then
                            If rowIndex <> index Then
                                ...
                                Dim Movimiento = New Salida.MovimientoCashOut
        ...
        Private Shared Function sconfigurarMovimientosCashOut(peticion As ContractoServicio.Contractos.Integracion.AltaMovimientosCashOut.Peticion, ByRef pMovimiento As Dictionary(Of Integer, Entrada.MovimientoCashOut), identificadorLlamada As String) As SPWrapper
            Dim SP As String = String.Format("SAPR_PSERVICIO_{0}.sgrabar_mov_cashout", Prosegur.Genesis.Comon.Util.Version)
            ...
            For Each mov In peticion.Movimientos
                If pMovimiento Is Nothing Then pMovimiento = New Dictionary(Of Integer, Entrada.MovimientoCashOut)

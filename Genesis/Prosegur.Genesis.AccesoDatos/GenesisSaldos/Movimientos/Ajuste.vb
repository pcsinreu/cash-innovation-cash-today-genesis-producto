...
Imports Prosegur.Genesis.ContractoServicio.Contractos.Integracion.AltaMovimientosAjuste.Salida
...
Namespace GenesisSaldos.Movimientos
    Public Class Ajuste
        ...
        Public Shared Sub AltaMovimientosAjuste(identificadorLlamada As String, peticion As Peticion, respuesta As Respuesta,
            ...
            Try
                ...
                If peticion.Movimientos IsNot Nothing AndAlso peticion.Movimientos.Count > 0 Then
                    ...
                    TiempoParcial = Now
                    Dim movimientosIndex As New Dictionary(Of Integer, Entrada.MovimientoAjuste)
                    spw = sconfigurarMovimientosAjuste(identificadorLlamada, peticion, movimientosIndex)
                    ...
                    TiempoParcial = Now
                    PoblarMovimientosAjuste(ds, respuesta, movimientosIndex)
        ...
        End Sub
        Private Shared Function sconfigurarMovimientosAjuste(identificadorLlamada As String, peticion As Peticion, ByRef movimientosIndex As Dictionary(Of Integer, Entrada.MovimientoAjuste)) As SPWrapper

            Dim SP As String = String.Format("SAPR_PSERVICIO_{0}.sgrabar_mov_ajuste", Prosegur.Genesis.Comon.Util.Version)
            ...
            spw.AgregarParam("par$acod_actual", ProsegurDbType.Identificador_Alfanumerico, Nothing, , True)


            spw.AgregarParam("par$anel_saldo_fisico", ProsegurDbType.Logico, Nothing, , True)
            ...
            For Each mov In peticion.Movimientos
                ...
                If movimientosIndex Is Nothing Then movimientosIndex = New Dictionary(Of Integer, Entrada.MovimientoAjuste)
                ...
                spw.Param("par$acod_actual").AgregarValorArray(mov.ActualId)




                spw.Param("par$anel_saldo_fisico").AgregarValorArray(If(mov.ImpactarSaldoFisico, 1, 0))
        ...
        End Function
        Private Shared Sub PoblarMovimientosAjuste(ds As DataSet, respuesta As Respuesta, movimientosIndex As Dictionary(Of Integer, Entrada.MovimientoAjuste))

            If respuesta.Movimientos Is Nothing Then respuesta.Movimientos = New List(Of MovimientoAjuste)
                    ...
                    For Each row As DataRow In ds.Tables("validaciones").DefaultView.ToTable().Rows
                        ...
                        If Not (rowIndex = 2040010027 Or rowIndex = 2040010026) Then
                            If rowIndex <> index Then
                                ...
                                Dim Movimiento = New MovimientoAjuste
                                ...
                                Movimiento.CollectionId = New List(Of String)
        ...
        Private Shared Function ObtenerCollectionIDs(detalles As List(Of Entrada.Detalle)) As List(Of String)
            Dim collectionsIDs As New List(Of String)

            For Each det In detalles
                If Not String.IsNullOrWhiteSpace(det.CollectionId) AndAlso Not collectionsIDs.Contains(det.CollectionId) Then
                    collectionsIDs.Add(det.CollectionId)
                End If
            Next

...
Namespace GenesisSaldos.Movimientos
    Public Class Balance
        Public Shared Sub AltaMovimientosBalance(identificadorLlamada As String, peticion As Peticion, respuesta As Respuesta, log As StringBuilder)
            ...
            Try
                ...
                If peticion.Movimientos IsNot Nothing AndAlso peticion.Movimientos.Count > 0 Then
                    TiempoParcial = Now
                    Dim movimientosIndex As New Dictionary(Of Integer, Entrada.MovimientoBalance)
                    spw = sconfigurarMovimientosBalance(identificadorLlamada, peticion, movimientosIndex)
                    ...
                    TiempoParcial = Now
                    PoblarMovimientosBalance(ds, respuesta, movimientosIndex)
        ...
        Private Shared Sub PoblarMovimientosBalance(ds As DataSet, respuesta As Respuesta, movimientosIndex As Dictionary(Of Integer, Entrada.MovimientoBalance))
            If respuesta.Movimientos Is Nothing Then respuesta.Movimientos = New List(Of Salida.MovimientoBalance)
                    ...
                    For Each row As DataRow In ds.Tables("validaciones").DefaultView.ToTable().Rows
                        ...
                        If Not (rowIndex = 2040010027 Or rowIndex = 2040010026) Then
                            If rowIndex <> index Then
                                ...
                                Dim Movimiento = New Salida.MovimientoBalance
        ...
        End Function
        Private Shared Function sconfigurarMovimientosBalance(identificadorLlamada As String, peticion As Peticion, ByRef pMovimiento As Dictionary(Of Integer, Entrada.MovimientoBalance)) As SPWrapper
            Dim SP As String = String.Format("SAPR_PSERVICIO_{0}.sgrabar_mov_balance", Prosegur.Genesis.Comon.Util.Version)
            ...
            For Each mov In peticion.Movimientos
                If pMovimiento Is Nothing Then pMovimiento = New Dictionary(Of Integer, Entrada.MovimientoBalance)

...
Namespace GenesisSaldos.Movimientos
    Public Class ShipIn
        ...
        Public Shared Sub AltaMovimientosShipIn(identificadorLlamada As String, peticion As Peticion, respuesta As Respuesta, log As StringBuilder)
            ...
            Try
                ...
                If peticion.Movimientos IsNot Nothing AndAlso peticion.Movimientos.Count > 0 Then
                    TiempoParcial = Now
                    Dim movimientosIndex As New Dictionary(Of Integer, Entrada.MovimientoShipIn)
                    spw = sconfigurarMovimientosShipIn(identificadorLlamada, peticion, movimientosIndex)
                    ...
                    TiempoParcial = Now
                    PoblarMovimientosShipIn(ds, respuesta, movimientosIndex)
        ...
        Private Shared Sub PoblarMovimientosShipIn(ds As DataSet, respuesta As Respuesta, movimientosIndex As Dictionary(Of Integer, Entrada.MovimientoShipIn))
            If respuesta.Movimientos Is Nothing Then respuesta.Movimientos = New List(Of Salida.MovimientoShipIn)
                    ...
                    For Each row As DataRow In ds.Tables("validaciones").DefaultView.ToTable().Rows
                        ...
                        If Not (rowIndex = 2040010027 Or rowIndex = 2040010026) Then
                            If rowIndex <> index Then
                                ...
                                Dim Movimiento = New Salida.MovimientoShipIn
                                ...
                                Movimiento.CollectionId = ObtenerCollectionsIDs(movimientosIndex(rowIndex).Detalles)
        ...
        Private Shared Function ObtenerCollectionsIDs(detalles As List(Of Entrada.Detalle)) As List(Of String)
        ...
        Private Shared Function sconfigurarMovimientosShipIn(identificadorLlamada As String, peticion As Peticion, ByRef pMovimiento As Dictionary(Of Integer, Entrada.MovimientoShipIn)) As SPWrapper
            Dim SP As String = String.Format("SAPR_PSERVICIO_{0}.sgrabar_mov_shipin", Prosegur.Genesis.Comon.Util.Version)
            ...
            For Each mov In peticion.Movimientos
                If pMovimiento Is Nothing Then pMovimiento = New Dictionary(Of Integer, Entrada.MovimientoShipIn)

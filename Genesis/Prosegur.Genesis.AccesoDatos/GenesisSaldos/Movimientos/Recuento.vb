...
Imports Prosegur.Genesis.ContractoServicio.Contractos.Integracion.AltaMovimientosRecuento
Imports Prosegur.Genesis.ContractoServicio.Contractos.Integracion.AltaMovimientosRecuento.Salida
...
Namespace GenesisSaldos.Movimientos
    Public Class Recuento
        ...
        Public Shared Sub AltaMovimientosRecuento(identificadorLlamada As String, peticion As Peticion, respuesta As Respuesta,
            ...
            Try
                ...
                If peticion.Movimientos IsNot Nothing AndAlso peticion.Movimientos.Count > 0 Then
                    ...
                    TiempoParcial = Now
                    Dim movimientosIndex As New Dictionary(Of Integer, Entrada.MovimientoRecuento)
                    spw = sconfigurarMovimientosRecuento(identificadorLlamada, peticion, movimientosIndex)
                    ...
                    TiempoParcial = Now
                    PoblarMovimientosRecuento(ds, respuesta, movimientosIndex)
        ...
        End Sub
        Private Shared Function sconfigurarMovimientosRecuento(identificadorLlamada As String, peticion As Peticion, ByRef movimientosIndex As Dictionary(Of Integer, Entrada.MovimientoRecuento)) As SPWrapper

            Dim SP As String = String.Format("SAPR_PSERVICIO_{0}.sgrabar_mov_recuento", Prosegur.Genesis.Comon.Util.Version)
            ...
            spw.AgregarParam("par$acod_precinto", ProsegurDbType.Identificador_Alfanumerico, Nothing, , True)
            ...
            spw.AgregarParam("par$aefdoc_nel_tipo", ProsegurDbType.Inteiro_Curto, Nothing, , True)
            ...
            For Each mov In peticion.Movimientos
                ...
                If movimientosIndex Is Nothing Then movimientosIndex = New Dictionary(Of Integer, Entrada.MovimientoRecuento)
                ...
                spw.Param("par$acod_precinto").AgregarValorArray(mov.IdentificadorRecuento)
                ...
                If mov.Importes IsNot Nothing Then
                    ...
                    For Each movGroup In mov.Importes.GroupBy(Function(x) x.CodigoDivisa)
                        ...
                        For Each importe In movGroup
                            ...
                            spw.Param("par$aefdoc_nel_tipo").AgregarValorArray((importe.Tipo + 0))
                        Next
                        indexDiv = indexDiv + 1
                    Next
                End If
        ...
        End Function
        Private Shared Sub PoblarMovimientosRecuento(ds As DataSet, respuesta As Respuesta, movimientosIndex As Dictionary(Of Integer, Entrada.MovimientoRecuento))

            If respuesta.Movimientos Is Nothing Then respuesta.Movimientos = New List(Of MovimientoRecuento)
                    ...
                    For Each row As DataRow In ds.Tables("validaciones").DefaultView.ToTable().Rows
                        ...
                        If Not (rowIndex = 2040010027 Or rowIndex = 2040010026) Then
                            If rowIndex <> index Then
                                ...
                                Dim Movimiento = New MovimientoRecuento

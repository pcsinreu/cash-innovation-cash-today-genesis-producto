...
Imports Prosegur.Genesis.ContractoServicio.Contractos.Integracion.AltaMovimientosProvisionEfectivo
Imports Prosegur.Genesis.ContractoServicio.Contractos.Integracion.AltaMovimientosProvisionEfectivo.Salida
...
Namespace GenesisSaldos.Movimientos
    Public Class ProvisionEfectivo
        ...
        Public Shared Sub AltaMovimientosProvisionEfectivo(identificadorLlamada As String, peticion As Peticion, respuesta As Respuesta,
            ...
            Try
                ...
                If peticion.Movimientos IsNot Nothing AndAlso peticion.Movimientos.Count > 0 Then
                    ...
                    TiempoParcial = Now
                    Dim movimientosIndex As New Dictionary(Of Integer, Entrada.MovimientoProvisionEfectivo)
                    spw = sconfigurarMovimientosProvisionEfectivo(identificadorLlamada, peticion, movimientosIndex)
                    ...
                    TiempoParcial = Now
                    PoblarMovimientosProvisionEfectivo(ds, respuesta, movimientosIndex)
        ...
        End Sub
        Private Shared Function sconfigurarMovimientosProvisionEfectivo(identificadorLlamada As String, peticion As Peticion, ByRef movimientosIndex As Dictionary(Of Integer, Entrada.MovimientoProvisionEfectivo)) As SPWrapper

            Dim SP As String = String.Format("SAPR_PSERVICIO_{0}.sgrabar_mov_provisionefectivo", Prosegur.Genesis.Comon.Util.Version)
            ...
            spw.AgregarParam("par$acod_banco_capital", ProsegurDbType.Identificador_Alfanumerico, Nothing, , True)
            ...
            For Each mov In peticion.Movimientos
                ...
                If movimientosIndex Is Nothing Then movimientosIndex = New Dictionary(Of Integer, Entrada.MovimientoProvisionEfectivo)
                movimientosIndex.Add(index, mov)
                spw.Param("par$anel_index").AgregarValorArray(index)
                spw.Param("par$acod_banco_capital").AgregarValorArray(mov.CodigoBancoCapital)
        ...
        End Function
        Private Shared Sub PoblarMovimientosProvisionEfectivo(ds As DataSet, respuesta As Respuesta, movimientosIndex As Dictionary(Of Integer, Entrada.MovimientoProvisionEfectivo))

            If respuesta.Movimientos Is Nothing Then respuesta.Movimientos = New List(Of MovimientoProvisionEfectivo)
                    ...
                    For Each row As DataRow In ds.Tables("validaciones").DefaultView.ToTable().Rows
                        ...
                        If Not (rowIndex = 2040010027 Or rowIndex = 2040010026) Then
                            If rowIndex <> index Then
                                ...
                                Dim Movimiento = New MovimientoProvisionEfectivo
                                Movimiento.CodigoBancoCapital = movimientosIndex(rowIndex).CodigoBancoCapital
                                Movimiento.FechaHora = movimientosIndex(rowIndex).FechaHora
                                Movimiento.FechaContable = movimientosIndex(rowIndex).FechaContable
                                Movimiento.CollectionId = movimientosIndex(rowIndex).CollectionId

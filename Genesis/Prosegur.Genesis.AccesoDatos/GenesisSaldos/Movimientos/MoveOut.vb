...
Imports Prosegur.Genesis.ContractoServicio.Contractos.Integracion.AltaMovimientosMoveOut
...
Namespace GenesisSaldos.Movimientos
    Public Class MoveOut
        Public Shared Sub AltaMovimientosMoveOut(identificadorLlamada As String, peticion As Peticion, respuesta As Respuesta, log As StringBuilder)
            ...
            Try
                ...
                If peticion.Movimientos IsNot Nothing AndAlso peticion.Movimientos.Count > 0 Then
                    TiempoParcial = Now
                    Dim movimientosIndex As New Dictionary(Of Integer, Entrada.MovimientoMoveOut)
                    spw = sconfigurarMovimientosMoveOut(identificadorLlamada, peticion, movimientosIndex)
                    ...
                    TiempoParcial = Now
                    PoblarMovimientosMoveOut(ds, respuesta, movimientosIndex)
        ...
        Private Shared Sub PoblarMovimientosMoveOut(ds As DataSet, respuesta As Respuesta, movimientosIndex As Dictionary(Of Integer, Entrada.MovimientoMoveOut))
            If respuesta.Movimientos Is Nothing Then respuesta.Movimientos = New List(Of Salida.MovimientoMoveOut)
                    ...
                    For Each row As DataRow In ds.Tables("validaciones").DefaultView.ToTable().Rows
                        ...
                        If Not (rowIndex = 2040010027 Or rowIndex = 2040010026) Then
                            If rowIndex <> index Then
                                ...
                                Dim Movimiento = New Salida.MovimientoMoveOut
        ...
        Private Shared Function ObtenerCollectionIDs(detalles As List(Of Entrada.Detalle)) As List(Of String)
            ...
            If detalles IsNot Nothing Then
                For Each det In detalles
                    If Not String.IsNullOrWhiteSpace(det.CollectionId) AndAlso listaRetorno.Contains(det.CollectionId) Then
            ...
            Return listaRetorno
        End Function
        Private Shared Function sconfigurarMovimientosMoveOut(identificadorLlamada As String, peticion As Peticion, ByRef pMovimiento As Dictionary(Of Integer, Entrada.MovimientoMoveOut)) As SPWrapper
            Dim SP As String = String.Format("SAPR_PSERVICIO_{0}.sgrabar_mov_MoveOut", Prosegur.Genesis.Comon.Util.Version)
            ...
            For Each mov In peticion.Movimientos
                If pMovimiento Is Nothing Then pMovimiento = New Dictionary(Of Integer, Entrada.MovimientoMoveOut)

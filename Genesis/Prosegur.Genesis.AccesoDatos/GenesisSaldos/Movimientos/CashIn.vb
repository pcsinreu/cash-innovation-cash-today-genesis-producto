...
Namespace GenesisSaldos.Movimientos
    Public Class CashIn
        Public Shared Sub AltaMovimientosCashIn(identificadorLlamada As String, peticion As Peticion, respuesta As Respuesta, log As StringBuilder)
            ...
            Try
                ...
                If peticion.Movimientos IsNot Nothing AndAlso peticion.Movimientos.Count > 0 Then
                    TiempoParcial = Now
                    Dim movimientosIndex As New Dictionary(Of Integer, Entrada.MovimientoCashIn)
                    spw = sconfigurarMovimientosCashIn(identificadorLlamada, peticion, movimientosIndex)
                    ...
                    TiempoParcial = Now
                    PoblarMovimientosCashIn(ds, respuesta, movimientosIndex)
        ...
        Private Shared Sub PoblarMovimientosCashIn(ds As DataSet, respuesta As Respuesta, movimientosIndex As Dictionary(Of Integer, Entrada.MovimientoCashIn))
            If respuesta.Movimientos Is Nothing Then respuesta.Movimientos = New List(Of Salida.MovimientoCashIn)
                    ...
                    For Each row As DataRow In ds.Tables("validaciones").DefaultView.ToTable().Rows
                        ...
                        If Not (rowIndex = 2040010027 Or rowIndex = 2040010026) Then
                            If rowIndex <> index Then
                                ...
                                Dim Movimiento = New Salida.MovimientoCashIn
                        ...
                        Else
                            ...
                            If respuesta.Resultado.Detalles Is Nothing Then
                                respuesta.Resultado.Detalles = New List(Of ContractoServicio.Contractos.Integracion.Comon.Detalle)
                            End If
                            If Not respuesta.Resultado.Detalles.Any(Function(x) x.Codigo = codigo) Then
                                respuesta.Resultado.Detalles.Add(New ContractoServicio.Contractos.Integracion.Comon.Detalle() With {.Codigo = codigo, .Descripcion = descripcion})
                            End If
                        End If
                    Next
                End If
        ...
        Private Shared Function sconfigurarMovimientosCashIn(identificadorLlamada As String, peticion As ContractoServicio.Contractos.Integracion.AltaMovimientosCashIn.Peticion, ByRef pMovimiento As Dictionary(Of Integer, Entrada.MovimientoCashIn)) As SPWrapper
            Dim SP As String = String.Format("SAPR_PSERVICIO_{0}.sgrabar_mov_cashin", Prosegur.Genesis.Comon.Util.Version)
            ...
            spw.AgregarParam("par$acndoc_det_index", ProsegurDbType.Identificador_Alfanumerico, Nothing, , True)
            ...
            spw.AgregarParam("par$aefdoc_collection_id", ProsegurDbType.Identificador_Alfanumerico, Nothing, , True)
            ...
            For Each mov In peticion.Movimientos
                If pMovimiento Is Nothing Then pMovimiento = New Dictionary(Of Integer, Entrada.MovimientoCashIn)
                 ...
                 If mov.Detalles IsNot Nothing Then
                    ...
                    For Each det In mov.Detalles
                        If det.Importes IsNot Nothing Then
                            ...
                            spw.Param("par$acndoc_det_index").AgregarValorArray(String.Format("{0}{1}", det.CollectionId, indexDet))
                            ...
                            For Each importe In det.Importes
                                ...
                                spw.Param("par$aefdoc_collection_id").AgregarValorArray(String.Format("{0}{1}", det.CollectionId, indexDet))

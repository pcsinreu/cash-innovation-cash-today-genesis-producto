...
function ExibirMensagem(mensagem, titulo, scriptFechar, btnFechar, botoes) {
    ...
    var erro = "<div style='float: left;'><span class='ui-icon ui-icon-alert' style='float: left; margin: 0 7px 50px 0;'></span></div><div style='float: right; width: 91%'>" + mensagem + "</div>";
    ...
    $(document).ready(function () {
        ...
        if ($("#dialog-message").length == 0) {
            ...
            var buttons = $("#dialog-message").dialog("option", "buttons");
...
function AbrirCalendario(elemento, mostrarHoras, postbackOnClose) {
...
var genesisAlertCount = 0;
function genesisAlert(msg, erro, tipo, close) {
    if (msg == "") {
        genesisAlertCount--;
        if (genesisAlertCount == 0) {
...
function ExibirMensagemModalSinLoading(mensagem, titulo) {

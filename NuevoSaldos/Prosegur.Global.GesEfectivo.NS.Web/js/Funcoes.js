...
function AbrirPopupModalConsiderandoCaminhoPantalla(url, altura, largura, redimensionavel, nombre, reload) {

    var fullUrl = window.location.href;

    fullUrl = fullUrl.split("Pantallas/")[0];

    url = fullUrl + "Pantallas/" + url;

    AbrirPopupModal(url, altura, largura, redimensionavel, nombre, reload);
}

function AbrirPopupModal(url, altura, largura, redimensionavel, nombre, reload) {
            ...
            $(WindowRef).load(function () {
                $(WindowRef).unload(function () {
                    ...
                    if (valores != null && valores[1] == "SesionExpirada") {
                        ...
                        if (reload) {
                            console.log(WindowRef.returnValue); __doPostBack("__Page", WindowRef.returnValue);

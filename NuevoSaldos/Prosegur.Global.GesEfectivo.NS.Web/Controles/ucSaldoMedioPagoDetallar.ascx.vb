...
Imports Prosegur.Global.GesEfectivo.NuevoSaldos.Web.ucSaldoMedioPagoDetallar.ParametrosUcSaldoMedioPagoDetallar
Imports Prosegur.Genesis.Comon.Extenciones
Imports Prosegur.Framework.Dicionario
Imports System.Collections.ObjectModel

Public Class ucSaldoMedioPagoDetallar
    Inherits UcBase
        ...
        Get
            If _DivisasDisponiveis Is Nothing Then
                ...
                If Divisas IsNot Nothing AndAlso Divisas.Count > 0 Then
                    _DivisasDisponiveis = (From divisa In Genesis.LogicaNegocio.Genesis.Divisas.ObtenerDivisas(, (From d In Divisas Select d.CodigoISO).ToObservableCollection, , True, False, True, , True).OrderBy(Function(d) d.Descripcion) Select New DivisaJS(divisa)).ToObservableCollection()
                End If
...
#Region "Eventos"
    Private Sub Page_PreRender(sender As Object, e As System.EventArgs) Handles Me.PreRender
        ...
        Dim parametros As New ParametrosUcSaldoMedioPagoDetallar()
            ...
            For Each divisa In Divisas
                If divisa.MediosPago IsNot Nothing Then
                    For Each medioPago In divisa.MediosPago.OrderBy(Function(mp) mp.Tipo.ToString() & "_" & mp.Descripcion)
                        ...
                        If medioPago.Valores IsNot Nothing Then
                            ...
                            medioPago.Valores.ToList().ForEach(Sub(valor)
                                                                   cantidad += valor.Cantidad
                                                                   importe += valor.Importe
                                                                   color = valor.Color
                                                                   detallar = valor.Detallar
                                                                   If valor.Terminos IsNot Nothing AndAlso valor.Terminos.Count > 0 Then
                                                                       terminos.Add(valor.Terminos)
                                                                   End If
                                                               End Sub)
                            If (importe <> 0) Then
                                parametros.TotalesMedioPago.Add(New TotalMedioPago(divisa, medioPago, cantidad, importe, detallar, color, terminos, Nothing))
                            End If
                        End If
                    Next
                End If
                If divisa.ValoresTotalesTipoMedioPago IsNot Nothing AndAlso divisa.ValoresTotalesTipoMedioPago.Count > 0 Then
                    For Each vtotal In divisa.ValoresTotalesTipoMedioPago
                        If (vtotal.Importe <> 0) Then
                            parametros.TotalesMedioPago.Add(New TotalMedioPago(divisa, Nothing, 0, 0, False, Nothing, Nothing, vtotal))
                        End If
                    Next
                End If
            Next
        ...
        parametros.Dicionario = New Dictionary(Of String, String)() From {
            ...
            {"importepositivodetallar", Tradutor.Traduzir("055_importepositivodetallar")},
            {"importenegativodetallar", Tradutor.Traduzir("055_importenegativodetallar")}
        }

        parametros.SeparadorDecimal = MyBase._DecimalSeparador
        parametros.SeparadorMilhar = MyBase._MilharSeparador

        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "ucSaldoMedioPagoDetallar_Inicializar_" & Me.ClientID, "InicializaUcSaldoMedioPagoDetallar(" & JsonConvert.SerializeObject(parametros, New Converters.StringEnumConverter()) & ", '" & pnlTotalesMedioPago.ClientID & "', '" & Me.ClientID & "');", True)
...
#Region "Metodos"
    ...
    Public Function verificarTipoMedioPago(identificadorDivisa As String, identificadorMedioPago As String) As Boolean

        Return DivisasDisponiveis IsNot Nothing AndAlso DivisasDisponiveis.Count > 0 AndAlso DivisasDisponiveis.Exists(Function(e) e.Identificador = identificadorDivisa AndAlso e.MediosPago IsNot Nothing AndAlso e.MediosPago.Count > 0 AndAlso e.MediosPago.Exists(Function(ee) ee.Identificador = identificadorMedioPago))
...
#Region "Overrides"
    ...
    Protected Overrides Sub Inicializar()
        ...
        If Not ExhibirTitulo Then
            dvDivisasMedioPagoDetallar.Style.Item("margin-left") = "0px !important"
        End If
    End Sub
...
#Region "Clases"
    ...
    Public Class ParametrosUcSaldoMedioPagoDetallar
